...
import com.tntmodders.takumi.client.model.sp.ModelPlayerTHM;
...
public class RenderPlayerTHM extends RenderLivingBase<AbstractClientPlayer> {

    public RenderPlayerTHM(RenderManager renderManager) {
        super(renderManager, new ModelPlayerTHM(), 1F);
    ...
    public float getCreeperFlashIntensity(EntityPlayer player, float partialTicks) {
        if (player.getActivePotionEffect(MobEffects.SLOWNESS) == null ||
                player.getActivePotionEffect(MobEffects.SLOWNESS).getAmplifier() < 10 ||
                player.getActivePotionEffect(MobEffects.SLOWNESS).getDuration() > 30) {
            return 0;
        }
        return (30 - (float) player.getActivePotionEffect(MobEffects.SLOWNESS).getDuration()) / 28f;
    }
