...
import com.tntmodders.takumi.entity.mobs.EntityNightmareCreeper;
...
public class RenderNightmareCreeper<T extends EntityNightmareCreeper> extends RenderTakumiCreeper<T> {
    public RenderNightmareCreeper(RenderManager renderManagerIn) {
        super(renderManagerIn);
        this.addLayer(new LayerNightmareCreeper(this));
    }

    @SideOnly(Side.CLIENT)
    public static class ModelNightmareCreeper extends ModelBase {
        ...
        public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw,
            ...
            EntityNightmareCreeper entityguardian = (EntityNightmareCreeper) entityIn;
            ...
            for (int i = 0; i < 12; ++i) {
                ...
                this.guardianSpines[i].rotationPointX =
                        afloat3[i] * (1.0F + MathHelper.cos(ageInTicks * 1.5F + i) * 0.01F - f1);
                this.guardianSpines[i].rotationPointY =
                        16.0F + afloat4[i] * (1.0F + MathHelper.cos(ageInTicks * 1.5F + i) * 0.01F - f1);
                this.guardianSpines[i].rotationPointZ =
                        afloat5[i] * (1.0F + MathHelper.cos(ageInTicks * 1.5F + i) * 0.01F - f1);
            }

            this.guardianEye.rotationPointZ = -8.25F;
            Entity entity = Minecraft.getMinecraft().getRenderViewEntity();

            if (entityguardian.getAttackTarget() != null) {
                entity = entityguardian.getAttackTarget();
            }
    ...
    public static class LayerNightmareCreeper implements LayerRenderer {
        private static final ResourceLocation GUARDIAN_ELDER_TEXTURE = new ResourceLocation("textures/entity/guardian_elder.png");
        private final ModelNightmareCreeper creeper = new ModelNightmareCreeper();
        private final RenderNightmareCreeper renderer;

        public LayerNightmareCreeper(RenderNightmareCreeper render) {
            super();
