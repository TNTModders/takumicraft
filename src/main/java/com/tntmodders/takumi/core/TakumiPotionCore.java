...
public class TakumiPotionCore {
    ...
    public static void registerPotionType(IForgeRegistry<PotionType> event) {
        PotionType type =
                new PotionType(SUBSIDENCE.getRegistryName().getResourcePath(), new PotionEffect(SUBSIDENCE, 400))
                        .setRegistryName(SUBSIDENCE.getRegistryName());
        event.register(type);
        type = new PotionType(INVERSION.getRegistryName().getResourcePath(), new PotionEffect(INVERSION, 400))
                .setRegistryName(INVERSION.getRegistryName());
        event.register(type);
    }
}
