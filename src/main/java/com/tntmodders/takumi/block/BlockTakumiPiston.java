...
public class BlockTakumiPiston extends BlockPistonBase {
    private final boolean isSticky;

    public BlockTakumiPiston(boolean isSticky) {
        super(isSticky);
        this.isSticky = isSticky;
        String s = isSticky ? "creeperpiston_sticky" : "creeperpiston";
    ...
    public boolean eventReceived(IBlockState state, World worldIn, BlockPos pos, int id, int param) {
        ...
        if (id == 0) {
            ...
            worldIn.setBlockState(pos, TakumiBlockCore.CREEPER_PISTON_EXTEND.getDefaultState().withProperty(BlockPistonMoving.FACING, enumfacing).withProperty(BlockPistonMoving.TYPE, this.isSticky ? BlockPistonExtension.EnumPistonType.STICKY : BlockPistonExtension.EnumPistonType.DEFAULT), 3);
            worldIn.setTileEntity(pos, BlockTakumiPistonMoving.createTilePiston(this.getStateFromMeta(param), enumfacing, false, true));
            ...
            if (this.isSticky) {
                ...
                if (block == TakumiBlockCore.CREEPER_PISTON_EXTEND) {
                ...
                if (!flag1 && !iblockstate.getBlock().isAir(iblockstate, worldIn, blockpos) && canPush(iblockstate, worldIn, blockpos, enumfacing.getOpposite(), false, enumfacing) &&
                        (iblockstate.getMobilityFlag() == EnumPushReaction.NORMAL || block == TakumiBlockCore.CREEPER_PISTON || block == TakumiBlockCore.CREEPER_STICKY_PISTON)) {
    ...
    private boolean doMove(World worldIn, BlockPos pos, EnumFacing direction, boolean extending) {
        ...
        if (!blockpistonstructurehelper.canMove()) {
            ...
            for (int l = list.size() - 1; l >= 0; --l) {
                ...
                worldIn.setBlockState(blockpos3, TakumiBlockCore.CREEPER_PISTON_EXTEND.getDefaultState().withProperty(FACING, direction), 4);
                worldIn.setTileEntity(blockpos3, BlockTakumiPistonMoving.createTilePiston(list1.get(l), direction, extending, false));
                --k;
            ...
            if (extending) {
                ...
                IBlockState iblockstate3 = TakumiBlockCore.CREEPER_PISTON_HEAD.getDefaultState().withProperty(BlockPistonExtension.FACING, direction).withProperty(BlockPistonExtension.TYPE, blockpistonextension$enumpistontype);
                IBlockState iblockstate1 = TakumiBlockCore.CREEPER_PISTON_EXTEND.getDefaultState().withProperty(BlockPistonMoving.FACING, direction).withProperty(BlockPistonMoving.TYPE, this.isSticky ? BlockPistonExtension.EnumPistonType.STICKY : BlockPistonExtension.EnumPistonType.DEFAULT);
                worldIn.setBlockState(blockpos2, iblockstate1, 4);
                worldIn.setTileEntity(blockpos2, BlockTakumiPistonMoving.createTilePiston(iblockstate3, direction, true, true));
            }
            ...
            if (extending) {
                worldIn.notifyNeighborsOfStateChange(blockpos2, TakumiBlockCore.CREEPER_PISTON_HEAD, false);
            }
