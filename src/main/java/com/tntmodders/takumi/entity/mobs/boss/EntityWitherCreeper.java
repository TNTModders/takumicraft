...
public class EntityWitherCreeper extends EntityTakumiAbstractCreeper implements IRangedAttackMob {
    ...
    protected void initEntityAI() {
        ...
        this.tasks.addTask(0, new EntityWitherCreeper.AIDoNothing());
    ...
    private void launchWitherSkullToCoords(int p_82209_1_, double x, double y, double z, boolean invulnerable) {
        ...
        double d4 = y - d1;
        double d5 = z - d2;
        EntityWitherCreeperSkull entitywitherskull = new EntityWitherCreeperSkull(this.world, this, d3, d4, d5);
    ...
    public boolean attackEntityFrom(DamageSource source, float amount) {
        if (this.isEntityInvulnerable(source)) {
            return false;
        } else if (source != DamageSource.DROWN && !(source.getTrueSource() instanceof EntityWitherCreeper)) {
    ...
    public void setDead() {
        if (!(this.getHealth() <= 0 || this.world.getDifficulty() == EnumDifficulty.PEACEFUL)) {
            if (!this.world.isRemote) {
                EntityWitherCreeper witherCreeper = new EntityWitherCreeper(this.world);
                NBTTagCompound tagCompound = new NBTTagCompound();
                this.writeEntityToNBT(tagCompound);
                tagCompound.setBoolean("ignited", false);
                witherCreeper.readEntityFromNBT(tagCompound);
                witherCreeper.setHealth(this.getHealth());
                witherCreeper.copyLocationAndAnglesFrom(this);
                if (this.getPowered()) {
                    TakumiUtils.takumiSetPowered(witherCreeper, true);
                }
                witherCreeper.setCreeperState(-1);
                witherCreeper.setAttackTarget(null);
                this.world.spawnEntity(witherCreeper);
            }
        }
        super.setDead();
    }
    ...
    public Object getRender(RenderManager manager) {
        return new RenderWitherCreeper<>(manager);
    }
}
