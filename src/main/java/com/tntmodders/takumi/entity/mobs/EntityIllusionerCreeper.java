...
public class EntityIllusionerCreeper extends EntityAbstractSpellCreeper implements IRangedAttackMob {
        ...
        if (this.isInvisible()) {
            ...
            if (this.hurtTime != 1 && this.ticksExisted % 1200 != 0) {
                if (this.hurtTime == this.maxHurtTime - 1) {
                    ...
                    for (int k = 0; k < 4; ++k) {
                        if (!this.world.isRemote) {
                            this.world.createExplosion(this, this.posX + this.renderLocations[0][k].x, this.posY + this.renderLocations[0][k].y,
                                    this.posZ + this.renderLocations[0][k].z, 2f, true);
                        }
                ...
                for (int j = 0; j < 4; ++j) {
                    ...
                    this.renderLocations[1][j] = new Vec3d((double) (-6.0F + (float) this.rand.nextInt(13)) * 0.5D, (double) Math.max(0, this.rand
                            .nextInt(6) - 4), (double) (-6.0F + (float) this.rand.nextInt(13)) * 0.5D);
                }
                
                for (int l = 0; l < 16; ++l) {
                    this.world.spawnParticle(EnumParticleTypes.CLOUD, this.posX + (this.rand.nextDouble() - 0.5D) * (double) this.width, this.posY
                            + this.rand.nextDouble() * (double) this.height, this.posZ + (this.rand.nextDouble() - 0.5D) * (double) this.width,
                            0.0D, 0.0D, 0.0D);
                }
                
                this.world.playSound(this.posX, this.posY, this.posZ, SoundEvents.ENTITY_ILLAGER_MIRROR_MOVE, this.getSoundCategory(), 1.0F, 1.0F,
                        false);
            }
        }
    }
    ...
    public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, IEntityLivingData livingdata) {
        ...
        this.setItemStackToSlot(EntityEquipmentSlot.OFFHAND, new ItemStack(TakumiItemCore.TAKUMI_ARROW_HA, this.rand.nextInt(15) + 1));
    ...
    public void attackEntityWithRangedAttack(EntityLivingBase target, float distanceFactor) {
        ...
        entityarrow.setThrowableHeading(d0, d1 + d3 * 0.20000000298023224D, d2, 1.6F, (float) (14 - this.world.getDifficulty().getDifficultyId() *
                4));
    ...
    class AIBlindnessSpell extends AIUseSpell {
        ...
        public boolean shouldExecute() {
            return super.shouldExecute() && EntityIllusionerCreeper.this.getAttackTarget() != null && EntityIllusionerCreeper.this.getAttackTarget
                    ().getEntityId() != this.lastTargetId && EntityIllusionerCreeper.this.world.getDifficultyForLocation(new BlockPos
                    (EntityIllusionerCreeper.this)).isHarderThan((float) EnumDifficulty.NORMAL.ordinal());
        }
        ...
        protected void castSpell() {
            ...
            if (!EntityIllusionerCreeper.this.world.isRemote) {
                EntityIllusionerCreeper.this.world.createExplosion(EntityIllusionerCreeper.this, EntityIllusionerCreeper.this.posX,
