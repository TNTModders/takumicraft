...
public class EntityAntinomyCreeper extends EntityTakumiAbstractCreeper {
    ...
    public void onLivingUpdate() {
        if (this.world.isRemote) {
            this.setFire(10);
            for (int i = 0; i < 4; ++i) {
                this.world.spawnParticle(EnumParticleTypes.SNOW_SHOVEL, this.posX + (this.rand.nextDouble() - 0.5D) * (double) this.width, this.posY + this.rand.nextDouble() * this.height, this.posZ + (this.rand.nextDouble() - 0.5D) * this.width, 0.0D, 0.0D, 0.0D);
                this.world.spawnParticle(EnumParticleTypes.FLAME, this.posX + (this.rand.nextDouble() - 0.5D) * this.width, this.posY + this.rand.nextDouble() * (double) this.height, this.posZ + (this.rand.nextDouble() - 0.5D) * this.width, 0.0D, 0.0D, 0.0D);
            }
        }
        ...
        for (j1 = 0; j1 < l; ++j1) {
            ...
            for (l1 = -k1; l1 <= k1; ++l1) {
                ...
                for (int i2 = -k1; i2 <= k1; ++i2) {
                    ...
                    if ((l1 == 0 && i2 == 0 || f1 * f1 + f2 * f2 <= f * f) && (l1 != -k1 && l1 != k1 && i2 != -k1 && i2 != k1 || this.rand.nextFloat() <= 0.75F)) {
                        IBlockState blockState = world.getBlockState(new BlockPos(x + l1, y + j1, z + i2));
                        this.world.setBlockState(new BlockPos(x + l1, y + j1, z + i2), Blocks.PACKED_ICE.getDefaultState());
                        if (j1 != 0 && k1 > 1) {
                            this.world.setBlockState(new BlockPos(x + l1, y - j1, z + i2), Blocks.PACKED_ICE.getDefaultState());
                        }
                    }
