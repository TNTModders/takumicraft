...
public class EntitySeaGuardianCreeper extends EntityTakumiAbstractCreeper {
    ...
    private static final DataParameter <Boolean> MOVING = EntityDataManager.createKey(EntitySeaGuardianCreeper.class, DataSerializers.BOOLEAN);
    private static final DataParameter <Integer> TARGET_ENTITY = EntityDataManager.createKey(EntitySeaGuardianCreeper.class, DataSerializers.VARINT);
...
/*        EntityRegistry.addSpawn(EntitySeaGuardianCreeper.class, this.takumiRank().getSpawnWeight(), 1, 3, TakumiEntityCore.WATER_TAKUMI, Biomes
    ...
    public boolean isNotColliding() {
        return this.world.checkNoEntityCollision(this.getEntityBoundingBox(), this) && this.world.getCollisionBoxes(this, this.getEntityBoundingBox
                ()).isEmpty();
    }
    ...
    public void onLivingUpdate() {
        if (this.world.isRemote) {
            ...
            if (this.isMoving() && this.isInWater()) {
                ...
                for (int i = 0; i < 2; ++i) {
                    this.world.spawnParticle(EnumParticleTypes.WATER_BUBBLE, this.posX + (this.rand.nextDouble() - 0.5D) * this.width - vec3d.x *
                            1.5D, this.posY + this.rand.nextDouble() * this.height - vec3d.y * 1.5D, this.posZ + (this.rand.nextDouble() - 0.5D) *
                            this.width - vec3d.z * 1.5D, 0.0D, 0.0D, 0.0D);
                }
            }
            ...
            if (this.hasTargetedEntity()) {
                ...
                if (entitylivingbase != null) {
                    ...
                    while (d4 < d3) {
                        ...
                        this.world.spawnParticle(EnumParticleTypes.WATER_BUBBLE, this.posX + d0 * d4, this.posY + d1 * d4 + this.getEyeHeight(),
    ...
    public float getBlockPathWeight(BlockPos pos) {
        return this.world.getBlockState(pos).getMaterial() == Material.WATER ? 10.0F + this.world.getLightBrightness(pos) - 0.5F :
    ...
    static class AIGuardianAttack extends EntityAIBase {
        ...
        public boolean shouldContinueExecuting() {
            return super.shouldContinueExecuting() && this.guardian.getDistanceSqToEntity(this.guardian.getAttackTarget()) > 9.0D;
        }
