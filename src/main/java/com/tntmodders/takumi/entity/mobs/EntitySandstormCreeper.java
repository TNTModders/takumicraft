...
public class EntitySandstormCreeper extends EntityTakumiAbstractCreeper {
    ...
    public void onLivingUpdate() {
        if (this.world.isRemote) {
            this.world.spawnParticle(EnumParticleTypes.CRIT, this.posX + (this.rand.nextDouble() - 0.5D) * (double) this.width,
    ...
    public boolean takumiExplodeEvent(ExplosionEvent.Detonate event) {
        ...
        for (BlockPos pos : event.getAffectedBlocks()) {
            for (int i = 0; i < 6; i++) {
                ...
                if (event.getWorld().getBlockState(newPos).getBlock() != Blocks.SAND && event.getWorld().getBlockState(newPos).getBlockHardness(world,
                                                                                                                                                newPos) != -1 && (event.getWorld().getBlockState(
                        newPos).getBlock().getExplosionResistance(world, newPos, event.getExplosion().getExplosivePlacedBy(),
