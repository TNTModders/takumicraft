...
/*        if (!this.world.isRemote && this.getAttackTarget() instanceof EntityAttackBlock && this.isGlowing()) {
        ...
        if (this.world.isAreaLoaded(blockpos$pooledmutableblockpos, blockpos$pooledmutableblockpos1)) {
            for (int i = blockpos$pooledmutableblockpos.getX(); i <= blockpos$pooledmutableblockpos1.getX(); ++i) {
                for (int j = blockpos$pooledmutableblockpos.getY(); j <= blockpos$pooledmutableblockpos1.getY(); ++j) {
                    for (int k = blockpos$pooledmutableblockpos.getZ(); k <= blockpos$pooledmutableblockpos1.getZ();
                         ++k) {
    ...
    class AIMoveRandom extends EntityAIBase {
        ...
        public boolean shouldExecute() {
            return !EntityLapisCreeper.this.getMoveHelper().isUpdating() &&
                    EntityLapisCreeper.this.rand.nextInt(7) == 0;
        }
