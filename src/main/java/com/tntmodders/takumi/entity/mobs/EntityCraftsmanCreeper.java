        ...
        event.getAffectedBlocks().forEach(pos -> {
            if (!this.world.isAirBlock(pos)) {
                ...
                if (event.getWorld().getTileEntity(pos) instanceof TileEntityTakumiCreepered) {
                    ((TileEntityTakumiBlock) event.getWorld().getTileEntity(pos)).setPath(state.getBlock().getRegistryName().toString());
                    ((TileEntityTakumiBlock) event.getWorld().getTileEntity(pos)).setMeta(state.getBlock().getMetaFromState(state));
                }
            }
        });
