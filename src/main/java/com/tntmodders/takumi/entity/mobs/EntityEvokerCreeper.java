...
public class EntityEvokerCreeper extends EntityAbstractSpellCreeper {
    ...
    class AIAttackSpell extends AIUseSpell {
        ...
        protected void castSpell() {
            ...
            float f = (float) MathHelper.atan2(entitylivingbase.posZ - EntityEvokerCreeper.this.posZ, entitylivingbase.posX - EntityEvokerCreeper
                    .this.posX);
            
            if (EntityEvokerCreeper.this.getDistanceSqToEntity(entitylivingbase) < 9.0D) {
                for (int i = 0; i < 5; ++i) {
                    float f1 = f + (float) i * (float) Math.PI * 0.4F;
                    this.spawnFangs(EntityEvokerCreeper.this.posX + (double) MathHelper.cos(f1) * 1.5D, EntityEvokerCreeper.this.posZ + (double)
                            MathHelper.sin(f1) * 1.5D, d0, d1, f1, 0);
                }
                
                for (int k = 0; k < 8; ++k) {
                    float f2 = f + (float) k * (float) Math.PI * 2.0F / 8.0F + (float) Math.PI * 2F / 5F;
                    this.spawnFangs(EntityEvokerCreeper.this.posX + (double) MathHelper.cos(f2) * 2.5D, EntityEvokerCreeper.this.posZ + (double)
                            MathHelper.sin(f2) * 2.5D, d0, d1, f2, 3);
                }
            } else {
                for (int l = 0; l < 16; ++l) {
                    double d2 = 1.25D * (double) (l + 1);
                    this.spawnFangs(EntityEvokerCreeper.this.posX + (double) MathHelper.cos(f) * d2, EntityEvokerCreeper.this.posZ + (double)
                            MathHelper.sin(f) * d2, d0, d1, f, l);
                }
            }
        }
        ...
        private void spawnFangs(double p_190876_1_, double p_190876_3_, double p_190876_5_, double p_190876_7_, float p_190876_9_, int p_190876_10_) {
            ...
            while (true) {
                if (!EntityEvokerCreeper.this.world.isBlockNormalCube(blockpos, true) && EntityEvokerCreeper.this.world.isBlockNormalCube(blockpos
                        .down(), true)) {
                    if (!EntityEvokerCreeper.this.world.isAirBlock(blockpos)) {
                        ...
                        AxisAlignedBB axisalignedbb = iblockstate.getCollisionBoundingBox(EntityEvokerCreeper.this.world, blockpos);
            ...
            if (flag) {
                EntityEvokerCreeperFangs EntityEvokerCreeperfangs = new EntityEvokerCreeperFangs(EntityEvokerCreeper.this.world, p_190876_1_,
                        (double) blockpos.getY() + d0, p_190876_3_, p_190876_9_, p_190876_10_, EntityEvokerCreeper.this);
    ...
    class AICastingSpell extends AICastingApell {
        ...
        public void updateTask() {
            if (EntityEvokerCreeper.this.getAttackTarget() != null) {
                EntityEvokerCreeper.this.getLookHelper().setLookPositionWithEntity(EntityEvokerCreeper.this.getAttackTarget(), (float)
                        EntityEvokerCreeper.this.getHorizontalFaceSpeed(), (float) EntityEvokerCreeper.this.getVerticalFaceSpeed());
            } else if (EntityEvokerCreeper.this.getWololoTarget() != null) {
                EntityEvokerCreeper.this.getLookHelper().setLookPositionWithEntity(EntityEvokerCreeper.this.getWololoTarget(), (float)
                        EntityEvokerCreeper.this.getHorizontalFaceSpeed(), (float) EntityEvokerCreeper.this.getVerticalFaceSpeed());
            }
        }
    }
    ...
    public class AIWololoSpell extends AIUseSpell {
        ...
        public boolean shouldExecute() {
            ...
            List <EntitySheep> list = EntityEvokerCreeper.this.world.getEntitiesWithinAABB(EntitySheep.class, EntityEvokerCreeper.this
                    .getEntityBoundingBox().grow(16.0D, 4.0D, 16.0D), this.wololoSelector);
