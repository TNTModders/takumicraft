...
public class EntityLlamaCreeperSpit extends Entity implements IProjectile {
    
    public EntityLlamaCreeper owner;
    private NBTTagCompound ownerNbt;
    
    public EntityLlamaCreeperSpit(World worldIn) {
        super(worldIn);
    }
    
    public EntityLlamaCreeperSpit(World worldIn, EntityLlamaCreeper p_i47273_2_) {
        super(worldIn);
        this.owner = p_i47273_2_;
        this.setPosition(p_i47273_2_.posX - (double) (p_i47273_2_.width + 1.0F) * 0.5D * (double) MathHelper.sin(p_i47273_2_.renderYawOffset *
                0.017453292F), p_i47273_2_.posY + (double) p_i47273_2_.getEyeHeight() - 0.10000000149011612D, p_i47273_2_.posZ + (double)
                (p_i47273_2_.width + 1.0F) * 0.5D * (double) MathHelper.cos(p_i47273_2_.renderYawOffset * 0.017453292F));
        this.setSize(0.25F, 0.25F);
    }
    
    @SideOnly(Side.CLIENT)
    public EntityLlamaCreeperSpit(World worldIn, double x, double y, double z, double p_i47274_8_, double p_i47274_10_, double p_i47274_12_) {
        super(worldIn);
    ...
    private void restoreOwnerFromSave() {
        if (this.ownerNbt != null && this.ownerNbt.hasUniqueId("OwnerUUID")) {
            ...
            for (EntityLlamaCreeper entityllama : this.world.getEntitiesWithinAABB(EntityLlamaCreeper.class, this.getEntityBoundingBox().grow
                    (15.0D))) {
    ...
    private Entity getHitEntity(Vec3d p_190538_1_, Vec3d p_190538_2_) {
        Entity entity = null;
        List <Entity> list = this.world.getEntitiesWithinAABBExcludingEntity(this, this.getEntityBoundingBox().expand(this.motionX, this.motionY,
                this.motionZ).grow(1.0D));
        double d0 = 0.0D;
    ...
    public void onHit(RayTraceResult p_190536_1_) {
        ...
        if (!this.world.isRemote) {
            this.world.createExplosion(this.owner, this.posX, this.posY, this.posZ, 3, true);
            this.setDead();
        }
    }
}
