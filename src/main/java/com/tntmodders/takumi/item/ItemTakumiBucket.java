...
public class ItemTakumiBucket extends ItemBucket {
    ...
    public ActionResult <ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
        ...
        if (flag) {
            ...
            if (material == TakumiMaterial.TAKUMI_WATER && iblockstate.getValue(BlockLiquid.LEVEL) == 0) {
                ...
                worldIn.setBlockState(blockpos, Blocks.AIR.getDefaultState(), 11);
                playerIn.addStat(StatList.getObjectUseStats(this));
                return new ActionResult <>(EnumActionResult.SUCCESS, this.fillBucket(itemstack, playerIn, TakumiItemCore.TAKUMI_WATER_BUCKET));
            }
            return new ActionResult <>(EnumActionResult.FAIL, itemstack);
        }
        ...
        if (this.tryPlaceContainedLiquid(playerIn, worldIn, blockpos1)) {
            ...
            playerIn.addStat(StatList.getObjectUseStats(this));
            return !playerIn.capabilities.isCreativeMode ? new ActionResult(EnumActionResult.SUCCESS, new ItemStack(TakumiItemCore.TAKUMI_BUCKET)) :
    ...
    public boolean tryPlaceContainedLiquid(@Nullable EntityPlayer player, World worldIn, BlockPos posIn) {
        ...
        if (worldIn.provider.doesWaterVaporize() ) {
            ...
            for (int k = 0; k < 8; ++k) {
                worldIn.spawnParticle(EnumParticleTypes.SMOKE_LARGE, l + Math.random(), (double) i + Math.random(), (double) j + Math.random(),
                        0.0D, 0.0D, 0.0D);
            }
        } else {
            ...
            SoundEvent soundevent = /*this.containedBlock == Blocks.FLOWING_LAVA ? SoundEvents.ITEM_BUCKET_EMPTY_LAVA :*/
