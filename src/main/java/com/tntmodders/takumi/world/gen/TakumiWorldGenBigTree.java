...
public class TakumiWorldGenBigTree extends WorldGenAbstractTree {
    ...
    public TakumiWorldGenBigTree(boolean notify, IBlockState log, IBlockState leaf) {
        super(notify);
        this.log = log;
        this.leaf = leaf;
    }
            ...
            if (f >= 0.0F) {
                for (int l = 0; l < i; ++l) {
                    ...
                    if (this.checkBlockLine(blockpos, blockpos1) == -1) {
                        ...
                        double d4 =
                                (double) blockpos.getY() - Math.sqrt((double) (i1 * i1 + j1 * j1)) * this.branchSlope;
