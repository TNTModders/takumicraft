        ...
        for (blockY = oy; blockY <= oy + height; ++blockY) {
            for (blockX = ox - radius; blockX <= ox + radius; ++blockX) {
                ...
                for (int blockZ = oz - radius; blockZ <= oz + radius; ++blockZ) {
                        ...
                        for (int y = oy - 1; y > 0; y--) {
                            ...
                            if (blockId == Blocks.AIR) {
                                if (par2Random.nextBoolean()) {
                                    this.setBlockAndNotifyAdequately(par1World, new BlockPos(blockX, y, blockZ),
                                            blockTakumi.getDefaultState());
                                }
                            } else {
                                break;
                            }
                    ...
                    if (rand.nextBoolean()) {
                        this.setBlockAndNotifyAdequately(par1World, new BlockPos(blockX, blockY, blockZ),
                                blockTakumi.getDefaultState());
                    }
                }
            }
