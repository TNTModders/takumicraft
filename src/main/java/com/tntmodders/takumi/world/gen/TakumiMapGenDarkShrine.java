...
public class TakumiMapGenDarkShrine extends MapGenStructure {
    /**
    ...
    protected boolean canSpawnStructureAtCoords(int chunkX, int chunkZ) {
        ...
        Random random = this.world.setRandomSeed(k, l, 10387310);
        k = k * this.distance;
        l = l * this.distance;
        ...
        if (i == k && j == l) {
            boolean flag =
                    this.world.getBiomeProvider().areBiomesViable(i * 16 + 8, j * 16 + 8, 0, VILLAGE_SPAWN_BIOMES);
    ...
    protected StructureStart getStructureStart(int chunkX, int chunkZ) {
        return new TakumiMapGenDarkShrine.Start(this.world, this.rand, chunkX, chunkZ, this.size);
    }
    ...
    public static class Start extends StructureStart {
        ...
        public Start(World worldIn, Random rand, int x, int z, int size) {
            super(x, z);
            List<StructureTakumiDarkShrinePieces.PieceWeight> list =
                    StructureTakumiDarkShrinePieces.getStructureTakumiDarkShrineWeightedPieceList(rand, size);
            StructureTakumiDarkShrinePieces.Start structurevillagepieces$start =
                    new StructureTakumiDarkShrinePieces.Start(worldIn.getBiomeProvider(), 0, rand, (x << 4) + 2,
                            (z << 4) + 2, list, size);
            ...
            for (StructureComponent structurecomponent1 : this.components) {
                if (!(structurecomponent1 instanceof StructureTakumiDarkShrinePieces.Road)) {
                    ++k;
                }
            }
