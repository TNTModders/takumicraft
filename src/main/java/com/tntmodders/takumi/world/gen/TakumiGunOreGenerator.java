    ...
    private void generateOre(World world, Random random, int x, int z) {
        ...
        if (world.provider instanceof WorldProviderSurface) {
            for (int i = 0; i < 10; i++) {
                ...
                int genZ = z + random.nextInt(16); new WorldGenMinable(random.nextInt(50) == 0 ? TakumiBlockCore.DUMMY_GUNORE.getDefaultState() :
                                                                       TakumiBlockCore.GUNORE.getDefaultState(), 25).generate(world, random, new
                        BlockPos(genX, genY, genZ));
            }
        }
    }
}
