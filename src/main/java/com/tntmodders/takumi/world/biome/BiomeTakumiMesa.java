...
public class BiomeTakumiMesa extends AbstractBiomeTakumiWorld {

    protected static final IBlockState HARDENED_CLAY = TakumiBlockCore.TAKUMI_CLAY.getDefaultState();
    protected static final IBlockState STAINED_HARDENED_CLAY = TakumiBlockCore.TAKUMI_CLAY.getDefaultState();
    ...
    public BiomeTakumiMesa() {
        super("takumimesa", AbstractBiomeTakumiWorld.getBaseProperty("takumimesa"));
        this.brycePillars = true;
        this.hasForest = false;
        this.spawnableCreatureList.clear();
        this.topBlock = ORANGE_STAINED_HARDENED_CLAY;
    ...
    public void genTerrainBlocks(World worldIn, Random rand, ChunkPrimer chunkPrimerIn, int x, int z, double noiseVal) {
        ...
        if (this.brycePillars) {
            ...
            if (d0 > 0.0D) {
                ...
                double d2 =
                        Math.abs(this.pillarRoofNoise.getValue((double) i * 0.001953125D, (double) j * 0.001953125D));
                d4 = d0 * d0 * 2.5D;
        ...
        for (int j1 = 255; j1 >= 0; --j1) {
            if (chunkPrimerIn.getBlockState(l1, j1, k1).getMaterial() == Material.AIR && j1 < (int) d4) {
                chunkPrimerIn.setBlockState(l1, j1, k1, TakumiWorldChunkGenerator.STONE);
            }
            ...
            if (j1 <= rand.nextInt(5)) {
                ...
                if (iblockstate1.getMaterial() == Material.AIR) {
                    l = -1;
                } else if (iblockstate1.getBlock() == TakumiWorldChunkGenerator.STONE.getBlock()) {
                    if (l == -1) {
                        flag1 = false;

                        if (k <= 0) {
                            iblockstate = AIR;
                            iblockstate3 = TakumiWorldChunkGenerator.STONE;
                        ...
                        if (j1 >= i2 - 1) {
                            if (this.hasForest && j1 > 86 + k * 2) {
                                if (flag) {
                                    chunkPrimerIn.setBlockState(l1, j1, k1, TakumiWorldChunkGenerator.DIRT);
                                } else {
                                    chunkPrimerIn.setBlockState(l1, j1, k1, TakumiWorldChunkGenerator.GRASS);
                                }
                            } else if (j1 > i2 + 3 + k) {
                            ...
                            if (iblockstate3.getBlock() == STAINED_HARDENED_CLAY.getBlock()) {
    ...
    class Decorator extends BiomeDecorator {
        ...
        protected void generateOres(World worldIn, Random random) {
            ...
            if (TerrainGen.generateOre(worldIn, random, goldGen, chunkPos, EventType.GOLD)) {
